"""
March Audit — Test 02: Top-50 Universe Ablation Study
======================================================
Runs the ablation experiments for Table 7 in the paper:
  1. HAR-RV-X baseline (tech_pred only as Ridge α=1.0)
  2. HAR-RV-X + News (add news_risk_score)
  3. HAR-RV-X + Retail (add retail_risk_score)
  4. HAR-RV-X + News + Retail (linear, no calendar/momentum/interaction)
  5. RIVE full (all 10 features, Ridge α=100)
  6. RIVE without News Agent (remove news_risk_score + news_x_retail)
  7. RIVE without Retail Agent (remove retail_risk_score + news_x_retail)
  8. News + Retail only (no tech_pred, no momentum, no calendar)

All evaluated on the Top-50 universe with raw targets, cutoff 2023-01-01.
...
from scripts.scale_up.config_universes import TOP_50_ACTIVE
...
def build_features(targets, residuals, news, retail):
    """Build full feature set (mirrors Feb pipeline)."""
    df = targets.copy()
...
def run_config(df, feature_list, alpha, cutoff):
    """Train Ridge and return test R² and RMSE."""
...
def main():
    print("\n" + "=" * 72)
    print("  MARCH AUDIT — TEST 02: TOP-50 ABLATION STUDY")
    print("=" * 72)

    targets, residuals, news, retail = load_data()

    # Filter to Top-50 tickers
    top50_tickers = [t for t in TOP_50_ACTIVE if t in targets["ticker"].unique()]
    targets_t50 = targets[targets["ticker"].isin(top50_tickers)]
    print(f"\n  Top-50 tickers available: {len(top50_tickers)}")

    df = build_features(targets_t50, residuals, news, retail)
    ...
    # Save
    out_path = Path(__file__).parent / "top50_ablation_results.csv"
