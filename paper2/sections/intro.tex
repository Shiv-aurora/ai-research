...
matter most. On a point-in-time panel of the 100 largest US
...
crises: 78.6\% coverage through the 2020 COVID crash, 76.5\% through the
2025 tariff shock, and deficits in most of the eight named episodes we
evaluate. The
same failure appears in a zero-shot time-series foundation model: the native
prediction band of Chronos \citep{ansari2024chronos} is not merely too
narrow in stress but asymmetrically so, missing on the upside 14.6\% of
...
thresholds are updated by every stock in the panel, so a rare regime's
effective daily learning rate scales with the panel size rather than
with a single series (a mechanism we identify precisely in
Section~\ref{sec:ablations}: under the panel's strong cross-sectional
dependence, pooling operates through this rate scaling, supplied
automatically, rather than through information aggregation). Second, \emph{regime conditioning}:
...
panel: stress coverage rises from 82.9\% to 88.1\% with other regimes flat
...
layer, applied around the Chronos median, produces bands within 0.3
percentage points of nominal in every regime.

The deficit that remains is as informative as the repair. Coverage on the
\emph{second day} after a spike enters the stress regime stays 11--17
points below the 90\% promise under every conditioning scheme we
evaluated: finer regime partitions, freshness subgroups, the
option-implied VIX9D/VIX signal, and even a deliberately noncausal
hindsight calibrator given full-sample smoothed regime memberships
...
bounded-step tracker can follow, and the option-implied signal we
evaluated does not anticipate it either. We report this as a boundary
of the approach, with a mechanism-level explanation and an explicit
account of what covering it would cost.
...
truth probed empirically at 0.2 percentage points. And the evaluation
...
\item \textbf{The method.} A regime-conditional, panel-pooled, self-tuning
  online conformal layer with per-regime coverage guarantees
  that hold for arbitrary dependence (finite-horizon inequalities whose
  numerical content is evaluated in Remark~\ref{rem:numeric}), plus
  a one-sided head that maps to VaR
  (Sections~\ref{sec:method}--\ref{sec:theory}).
\item \textbf{The decomposition.} Ablations attributing the repair to its
  ...
  and the per-regime guarantee itself; adaptivity removes manual
  regime-specific learning-rate selection
...
\item \textbf{A persistent onset limitation.} The day-2 onset window
  resists every conditioning scheme and signal we evaluate, including
  hindsight regimes and the option-implied term structure; we
  characterize the evidence and its boundaries
