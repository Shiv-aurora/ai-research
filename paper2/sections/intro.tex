...
almost exactly --- and cover only 82.9\% of stress-regime days, while
...
state variables, with soft memberships handled natively. Third,
\emph{self-tuning adaptation}: per-regime learning rates are selected
online by an expert-aggregation scheme with an issued-interval corrector,
leaving no tuned parameter anywhere in the layer. The result on the same
...
layer flattens the Chronos band to within 0.2 percentage points of nominal
in every regime.
...
\item \textbf{Foundation-model repair.} Evidence that a zero-shot TSFM's
  ...
  the same layer repairs it --- timely, given that current TSFM
