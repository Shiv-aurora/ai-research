...
Table~\ref{tab:main} is the paper's central comparison: seven methods on
...
Three readings of Table~\ref{tab:main} matter. First, the repair is a
...
where baselines over-cover. Second, the reallocation is free by the
standard economic metric: a Model Confidence Set \citep{hansen2011model}
...
lowest mean loss) and eliminates SF-OGD ($p < .001$). Conditional validity
costs nothing in average interval score. Third, similarity weighting
...
(Section~\ref{sec:method}). With zero tuned parameters it ties the
