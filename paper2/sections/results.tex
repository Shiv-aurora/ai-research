...
Table~\ref{tab:main} is the paper's central comparison: eleven methods
...
algorithm), and the parameter-free group-conditional coin-betting method
of \citet{bharti2026parameterfree} (POGO, run per stock as its
single-stream setting specifies, with our hard VIX bins supplied as its
groups --- the exact interface its guarantee requires). Every marginal coverage sits
...
($[+2.8, +8.9]$, $p < .001$). We report both inferential treatments and
let them mean what they mean: the repair is era-dependent insurance,
overwhelming when fast regime shifts occur and free when they do not.
...
lowest mean loss) and eliminates SF-OGD, TCP-RM, XS-panel, and POGO
...
scores wholesale rather than conditioning on the regime. XS-panel
...
against RC, $t = 8.4$). This is not an indictment of the method --- the
