...
Table~\ref{tab:main} is the paper's central comparison: ten methods on
the identical common sample. The baselines span the online conformal
family (ACI, DtACI, SF-OGD), the recent rolling-conformal method of
...
from the $k = 250$ most similar past days by market state), and a
...
miscoverage level). Every marginal coverage sits
...
($t = 3.05$, $p = .002$), significant at the 1\% level against every
baseline except TCP-RM ($+3.1$ points, $p = .027$), while marginal
coverage differences among the conformal
...
Five readings of Table~\ref{tab:main} matter. First, pooling without
...
lowest mean loss) and eliminates SF-OGD, TCP-RM, and XS-panel
($p < .001$); we do not detect inferior average interval-score
performance. Fourth, similarity weighting
...
pooling operates inside the tracking loop. Fifth, the two most recently
proposed baselines bracket our design and fall short on opposite sides.
TCP-RM is the strongest per-stock method in stress (84.9\%, still $3.1$
points below RC, $t = 2.21$, $p = .027$) but pays for it with the widest
per-stock intervals, because its rolling 60-day window forgets calm-period
scores wholesale rather than conditioning on the regime. XS-panel has
access to the same cross-sectional information our method pools, yet its
stress coverage (82.7\%) is indistinguishable from ACI's: contemporaneous
cross-sections tell the calibrator how wide intervals should be
\emph{today}, but without per-regime tracking the adaptive level still
reacts only after misses accumulate. Cross-sectional information and
coverage feedback each fail separately; the gain comes from combining
them inside regime-conditional trackers.
...
method is fixed a priori (Appendix~\ref{app:algorithm}) and calibration is
...
\begin{table}[t]
\centering
\caption{Temporal robustness (E14): the same issued intervals evaluated
