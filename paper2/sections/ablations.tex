...
speed alone: a single stream updated at rate $0.2$ tracks its own noise,
whereas the pooled summed update takes $n_t$ small per-observation steps
per day --- the same daily speed at roughly $1/\bar n$ of the gradient
variance. Pooling's operative contribution is that per-observation
stepping supplies the panel-scaled rate \emph{and} the variance
reduction automatically, inside a standard specification; the shared
threshold additionally tightens the cross-sectional coverage
...
regime layer, on the identical calibrator with only $K$ varied. The
honest summary: on the interval side, the point estimates consistently
