...
train/test date assertion. Scale conventions, exactly: experts forecast
log variance; QLIKE is evaluated in \emph{variance} space by
exponentiating, $\mathrm{QLIKE} = e^{y - m} - (y - m) - 1$; the pool is
the weight-averaged forecast in \emph{log} space (a geometric mean of
variance forecasts), with no Jensen retransformation correction ---
any resulting location bias is absorbed by the calibration layer, which
wraps whatever point forecast it is given. The pool ties its components (QLIKE 0.191 vs
...
stress upper tail (Section~\ref{sec:onset}); it \emph{overrides} the VIX
bin on the days it fires, so the result is a five-state mutually
exclusive hard partition, still within the setting of
Section~\ref{sec:theory}. The calibrator consumes any
...
is the pool forecast and the scale is a strictly trailing robust estimate
of that stock's own \emph{raw residuals}:
$\hat\sigma_{i,t} = 1.4826 \cdot \mathrm{MAD}\bigl( y_{i,\tau} - m_{i,\tau}
: \tau \in [t - 250, t - 1] \bigr)$, lagged one day so the scale used on
day $t$ never includes day $t$'s residual, with a minimum history of 50
days and zero scales treated as missing. Standardization puts all stocks
on a comparable scale, which is what makes cross-sectional pooling
meaningful.
...
tracked independently because volatility errors are skewed, and issues
stock $i$ the interval $[L_{i,t}, U_{i,t}]$ with
\[
  L_{i,t} = m_{i,t} - \Bigl( \textstyle\sum_k \pi_t(k)\,
  q_t^{\mathrm{lo}}(k) \Bigr) \hat\sigma_{i,t},
  \qquad
  U_{i,t} = m_{i,t} + \Bigl( \textstyle\sum_k \pi_t(k)\,
  q_t^{\mathrm{hi}}(k) \Bigr) \hat\sigma_{i,t}.
\]
After observing outcomes, each side takes one \emph{summed
cross-sectional mini-batch} gradient step per day
(Section~\ref{sec:theory}, eq.~\eqref{eq:update}): every regime's
threshold moves proportionally to its membership and to the day's
summed coverage error across the panel, so the step size scales with
the number of stock-days the regime governs. This panel-scaled stepping
is the pooling mechanism: on a 100-name panel the stress regime's
cumulative movement budget corresponds to roughly $15{,}000$ stock-day
error contributions over the sample where a single stock would provide
$\sim$150 --- one batched step per day, not $15{,}000$ sequential
updates, and the informational content of the batch under
cross-sectional dependence is quantified in
Section~\ref{sec:mechanism-sims}. The headline
method has no per-stock parameters; a per-stock offset variant is
evaluated as an ablation and found negligible
...
(Proposition~\ref{prop:adaptive}); the bank carries adaptivity. This
configuration removes the manual selection of regime-specific conformal
learning rates, the choice we found most consequential and least
principled to make by hand; its remaining constants (the rate grid, the
aggregation rate, the mixing coefficient, and the corrector rate) are
fixed a priori, identical across all experiments, and listed in the
complete algorithm specification of \ref{app:algorithm}. Its
learned effective rates reproduce what
hand-tuning found: stress-regime rates roughly twice calm rates, and
1\%-tail rates roughly twice 5\%-tail rates
